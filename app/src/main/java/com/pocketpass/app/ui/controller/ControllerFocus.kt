...
private const val HIGHLIGHT_STROKE = 20f
private const val HIGHLIGHT_GLOW_BLUR = 36f
private const val HIGHLIGHT_GLOSS_BLUR = 16f
private val HighlightTop = Color(0xFFA2FF70)
private val HighlightBottom = Color(0xFF00E31B)
private const val HIGHLIGHT_GLOW_COLOR = 0x406FFF62
internal const val FOCUS_SLIDE_DAMPING_RATIO = 0.88f
internal const val FOCUS_SLIDE_STIFFNESS = 220f
...
fun ControllerFocusHighlight(focus: ControllerFocus, display: FocusDisplay? = FocusDisplay.Bottom) {
    ...
    LaunchedEffect(focus, display) {
        ...
        snapshotFlow { focusedTarget.value }.collect { next ->
            val previous = tracker.previousFor(next, focus.hidden, System.nanoTime())
    ...
    Canvas(Modifier.fillMaxSize()) {
        ...
        if (viewport == null || viewport.bounds.isEmpty) {
            drawHighlight(bounds, radius, target.scale, masks, glowPaint, glossPaint)
            return@Canvas
        }
...
private fun DrawScope.drawHighlight(
    ...
    glossPaint: android.graphics.Paint,
) {
    ...
    val destination = android.graphics.RectF(
        bounds.left - mask.pad,
        bounds.top - mask.pad,
        bounds.right + mask.pad,
        bounds.bottom + mask.pad,
    )
    val canvas = drawContext.canvas.nativeCanvas

    canvas.drawBitmap(mask.glow, null, destination, glowPaint)
    ...
    canvas.drawBitmap(mask.gloss, null, destination, glossPaint)
}

private class HighlightMask(
    val glow: android.graphics.Bitmap,
    val gloss: android.graphics.Bitmap,
    val pad: Float,
    val width: Int,
    val height: Int,
    val radius: Int,
...
private class HighlightMaskCache {
...
private fun buildHighlightMask(width: Float, height: Float, innerRadius: Float, scale: Float): HighlightMask {
    ...
    val bounds = android.graphics.RectF(pad * s, pad * s, (pad + width) * s, (pad + height) * s)
    fun inflated(amount: Float) = android.graphics.RectF(
        bounds.left - amount * s,
        bounds.top - amount * s,
        bounds.right + amount * s,
        bounds.bottom + amount * s,
    )
    fun roundRect(rect: android.graphics.RectF, radius: Float) = android.graphics.Path().apply {
        addRoundRect(rect, radius * s, radius * s, android.graphics.Path.Direction.CW)
    }
    val outerPath = roundRect(inflated(stroke), innerRadius + stroke)
    val innerPath = roundRect(bounds, innerRadius)

    val glow = android.graphics.Bitmap.createBitmap(maskWidth, maskHeight, android.graphics.Bitmap.Config.ALPHA_8)
    android.graphics.Canvas(glow).apply {
        clipOutPath(outerPath)
        val ring = inflated(stroke / 2f)
        drawRoundRect(
            ring,
            (innerRadius + stroke / 2f) * s,
            (innerRadius + stroke / 2f) * s,
            android.graphics.Paint(android.graphics.Paint.ANTI_ALIAS_FLAG).apply {
                style = android.graphics.Paint.Style.STROKE
                strokeWidth = stroke * s
                maskFilter = BlurMaskFilter((glowBlur * s).coerceAtLeast(0.5f), BlurMaskFilter.Blur.NORMAL)
            },
        )
    }

    val gloss = android.graphics.Bitmap.createBitmap(maskWidth, maskHeight, android.graphics.Bitmap.Config.ALPHA_8)
    android.graphics.Canvas(gloss).apply {
        ...
        clipPath(ringPath)
        val glossPath = android.graphics.Path().apply {
            fillType = android.graphics.Path.FillType.EVEN_ODD
            addRect(inflated(stroke + glossBlur * 3f), android.graphics.Path.Direction.CW)
            addPath(outerPath)
            addPath(innerPath)
        }
        drawPath(
            glossPath,
            android.graphics.Paint(android.graphics.Paint.ANTI_ALIAS_FLAG).apply {
                maskFilter = BlurMaskFilter((glossBlur * s).coerceAtLeast(0.5f), BlurMaskFilter.Blur.NORMAL)
            },
        )
    }

    return HighlightMask(glow, gloss, pad, width.roundToInt(), height.roundToInt(), innerRadius.roundToInt(), scale)
}

private const val HIGHLIGHT_MASK_SCALE = 0.25f
